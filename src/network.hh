...
public:
  ...
  NetConfig( const RemyBuffers::NetConfig & dna )
    : mean_on_duration( dna.mean_on_duration() ),
      ...
      num_tcp_senders( dna.num_tcp_senders() )
  {}
  ...
  NetConfig & set_num_tcp_senders( const double s ) { num_tcp_senders = s; return *this; }
  ...
  RemyBuffers::NetConfig DNA( void ) const
  {
      ...
      ret.set_num_tcp_senders( num_tcp_senders );
      return ret;
  }
  ...
  {
    char tmp[ 256 ];
    snprintf( tmp, 256, "mean_on=%f, mean_off=%f, nsrc=%f, link_ppt=%f, delay=%f, buffer_size=%f, stochastic_loss_rate = %f, num_tcp_senders=%f\n",
	     mean_on_duration, mean_off_duration, num_senders, link_ppt, delay, buffer_size, stochastic_loss_rate, num_tcp_senders );
    return tmp;
  }
};
