    ...
    _configs()
{
  ...
  while ( rtt <= rtt_max ) {
    _configs.push_back( NetConfig().set_link_ppt( link_speed ).set_delay( rtt ).set_num_senders( range.max_senders ).set_bdp_multiplier( range.bdp_multiplier ).set_on_duration( range.mean_on_duration ).set_off_duration( range.mean_off_duration ) );
    rtt += rtt_increment;
  }
}
